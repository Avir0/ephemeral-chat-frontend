...
// ❌ BUG: if VITE_API_BASE already ends with /api/rooms,
// you’ll get duplicated /api/rooms/api/rooms
// ✅ FIX: make sure VITE_API_BASE = server root only
// e.g. "https://ephemeral-chat-backend-3ikd.onrender.com"
const API_BASE = import.meta.env.VITE_API_BASE;
...
function ChatRoom({ roomId, username, onExit }) {
  ...
  useEffect(() => {
    // Anti-screenshot (optional, not foolproof)
    const onKey = (e) => {
      if (e.code === 'PrintScreen') {
        e.preventDefault();
        alert('Screenshots are discouraged on this page.');
    ...
    <div className="card secure-chat-container">
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button
            className="btn"
            onClick={() => navigator.clipboard.writeText(window.location.origin + '/chat/' + roomId)}
          >
            Copy Link
          </button>
          <button className="btn" onClick={onExit}>
      ...
      <div className="messages" id="messages">
        {messages.map((m) => (
          <div
            key={m._id}
            className={`msg ${m.sender === username ? 'me' : ''} ${m.system ? 'system' : ''}`}
          >
